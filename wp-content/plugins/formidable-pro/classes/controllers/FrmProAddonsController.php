...
class FrmProAddonsController extends FrmAddonsController {
	 ...
	 * @return void
	 */
	private static function show_warning_overlay_gpl_license( $error = array() ) {
		if ( ! class_exists( 'FrmOverlayController' ) ) {
			return;
		}
		...
		$overlay_wrapper = new FrmOverlayController(
			array(
				'config-option-name'  => 'gpl-license-warning',
		...
		$copy = sprintf(
			...
			'<br/> <a class="frm-gpl-license-green-cta" href="' . FrmAppHelper::admin_upgrade_link( 'gpl-full' ) . '" target="_blank">',
			'<b>',
			'</b></a>'
		);
		if ( isset( $error['message'] ) ) {
			$copy = str_replace( array( 'utm_medium=gpl', '50% off', '<a ', '</a>.' ), array( 'utm_medium=gpl-full', '<b>50% off</b>', '<a class="frm-gpl-license-green-cta" ', '</a>' ), html_entity_decode( $error['message'] ) );
		}
		...
		$overlay_wrapper->open_overlay(
			array(
				...
				'buttons'    => array(
					array(
						'url'    => FrmAppHelper::admin_upgrade_link( 'gpl-full', 'formidable-forms-pro-gpl/' ),
						'target' => '_blank',
						'label'  => esc_html__( 'Learn More', 'formidable' ),
					),
					array(
						'url'    => FrmAppHelper::admin_upgrade_link( 'gpl-full' ),
						'target' => '_blank',
	...
	public static function show_warning_overlay_for_expired_or_null_license() {
		...
		$inline_style = '.frm-overlay--copy .frm-gpl-license-green-cta, .frm-overlay--copy .frm-gpl-license-green-cta:hover {
			background: #14b66a;
		...
		if ( isset( $license_is_expired['type'] ) && 'invalid' === $license_is_expired['type'] ) {
			self::show_warning_overlay_gpl_license( $license_is_expired );
			return;
		}
